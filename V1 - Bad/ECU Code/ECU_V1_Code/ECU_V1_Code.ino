...
uint8_t valveClosed[14] = {30, 30, 10, 30, 30, 30, 45, 15, 15, 15, 20, 20, 30, 40};
uint8_t valveOpened[14] = {160, 160, 120, 160, 160, 160, 100, 100, 120, 120, 100, 100, 160, 170};
...
void loop()
{
  ...
  // Valve toggles, 1: toggle valve directly, 2: sets manually override, eg {1,1} or {2,5}
  ...
  if(commandInt == 1){
    ...
    if(valveIndex >= 0 && valveIndex <= 13){  // Checking that the index is within range
      if(valveOverride[valveIndex] == 1 && !isValveScheduled(valveIndex)){     // Checking that this valve is in manual control
        if(valveState[valveIndex] == 1){      // Gotta close it if open
          valveServos[valveIndex].write(valveClosed[valveIndex] - valveOvershoot[valveIndex]);
          scheduleValveAdjustment(2, valveIndex, 0);
          valveState[valveIndex] = 0;
        } else {                              // Gotta open if if closed
          valveServos[valveIndex].write(valveOpened[valveIndex] + valveOvershoot[valveIndex]);
  ...
  if(int(millis() / 1000.0) != lastTime){
    printValveStates();
  }
  
  lastTime = int(millis() / 1000.0);
...
float extractFloat(const char str[], int i) {
    ...
    while (str[i] != '\0' && j < 19) {
        if (isdigit(str[i]) || str[i] == '.' || str[i] == '-') {
            ...
            break;  // Stop when we hit a non-numeric character after starting
        }
        i++;
    }
...
void scheduleValveAdjustment(unsigned long delaySeconds, int valveID, int position) {
    for (int i = 0; i < MAX_TASKS; i++) {
    ...
    //Serial.println("Task list full! Cannot schedule new valve adjustment.");
}
    ...
    for (int i = 0; i < MAX_TASKS; i++) {
        if (tasks[i].active && currentTime >= tasks[i].triggerTime) {
          if(tasks[i].position == 0){
            valveServos[tasks[i].valveID].write(valveClosed[tasks[i].valveID]);
            // Serial.println("Adjust Closed");
          } else {
            valveServos[tasks[i].valveID].write(valveOpened[tasks[i].valveID]);
