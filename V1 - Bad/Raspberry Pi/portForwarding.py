...
daq_sensor_types = ["PT", "PT", "PT", "PT", "PT", "PT", "PT", "PT", "TC_T", "PT", "TC_T", "PT", "TC_K", "PT"]
        ...
        if i_sensor != 14:
          try:
            if daq_sensor_types[i_sensor] == "TC_K":
              ...
              processed_sample[i_sensor] = mean(sample[i_sensor])*1000000 /40 + processed_sample[14]#tc_k_reference.inverse_CmV(mean(sample[i_sensor])*1000, Tref=processed_sample[14])
            elif daq_sensor_types[i_sensor] == "TC_T":
              processed_sample[i_sensor] = mean(sample[i_sensor])*1000000/20 + processed_sample[14]#mean(sample[i_sensor])*1000#tc_t_reference.inverse_CmV(mean(sample[i_sensor])*1000, Tref=processed_sample[14])
...
filename = os.path.dirname(__file__)+'/Logs/' + str(datetime.now()).replace(" ", "_").replace(".", "_").replace(":", "_") + '.csv'
filename_2 = os.path.dirname(__file__)+'/Logs/' + str(datetime.now()).replace(" ", "_").replace(".", "_").replace(":", "_") + '_raw.csv'
