...
from core import U6_Pro_poller
...
def get_sensor_type(channel: int):
    dpg.get_value("type_U6p_" + str(channel))
...
def send_config_data(sender, app_data, user_data):
    ...
    for i in range(14):
        ...
        scan_rate = dpg.get_value("U6p_scan_rate_input")

    U6_Pro_poller.update_config(active_channels, active_channel_types, mappings, sensor_locations, scan_rate)


def generate_channel_config_string(channel: int):
    row_string = "U6p_CH" + str(channel)
    enabled = dpg.get_value(f"enabled_{row_string}")
...
def callback_save_config():
    ...
    save_config_data = str(dpg.get_value("U6p_scan_rate_input")) + "\n"
...
def callback_open_config_confirm(sender, app_data, user_data):
    try:
        ...
        for i_line in range(len(csv_lines)):
            ...
            info = line.split(",")
            if i_line == 0:
                dpg.set_value("U6p_scan_rate_input", float(info[0]))
            else:
                row_string = "U6p_CH" + str(i_line)
...
def load_defaults():
    ...
    file_path = this_file_dir.parent / "save_files" / "defaults" / "default_U6_Pro.csv"
    callback_open_config_confirm(None, {"file_path_name": file_path}, None)
    send_config_data(None, None, None)


def build():
    with dpg.collapsing_header(label="U6 Pro DAQ", default_open=True):
        ...
        with dpg.file_dialog(
            ...
            width=500,
            height=400,
            label="Load U6 Pro Config",
        ):
            dpg.add_file_extension(".csv", color=(255, 255, 255, 255))

        with dpg.group(horizontal=True):
            dpg.add_button(label="Apply To DAQ and Clear", callback=send_config_data, tag="U6p_apply")
            with dpg.tooltip("U6p_apply"):
                dpg.add_text(
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_input_int(default_value=5, width=150, tag="U6p_scan_rate_input")
            dpg.add_text("Hz")
